...
save_data_simple <- function(data, file_name, file_type, csv_method = getOption("corevoters.csv_writer", "readr")) {
  ...
  switch(tolower(file_type),  # Use tolower to make file type case-insensitive
         ...
         "csv" = {
           method <- tolower(if (is.null(csv_method) || is.na(csv_method) || csv_method == "") "readr" else csv_method)
           target <- paste0(file_name, ".csv")
           
           if (method == "arrow" && requireNamespace("arrow", quietly = TRUE)) {
             log_info("Writing CSV with arrow::write_csv_arrow to {target}")
             arrow::write_csv_arrow(data, target)
           } else {
             if (method == "arrow") {
               log_warn("csv_method set to 'arrow' but arrow not available; falling back to readr::write_csv for {target}")
             } else {
               log_info("Writing CSV with readr::write_csv to {target}")
             }
...
prune_data <- function(data, column_limit = 250, seed = 123) {
  ...
  if (ncol(data) > column_limit) {
    set.seed(seed)
    columns_included <- sample(names(data), column_limit, replace = FALSE)
    data <- data %>% select(columns_included)
  } else {
    # Log message before returning the data
    message("No columns were pruned.")
  }
  return(data)
}

# Enhanced helper function for extracting and converting value labels
convert_labels_replace <- function(data) {
  # Step 1: Process labelled columns
  is_labelled_column <- function(x) {
    inherits(x, "haven_labelled") || labelled::is.labelled(x)
  }
  
  lbl_columns <- names(data)[purrr::map_lgl(data, is_labelled_column)]
  
  if (length(lbl_columns) > 0) {
    message("Processing labelled columns:")
    # print(lbl_columns)
    for (col_name in lbl_columns) {
      # Check if all non-NA values in the column are numeric-like
      non_na_values <- as.character(data[[col_name]][!is.na(data[[col_name]])])
      if (inherits(data[[col_name]], "double") && all(grepl("^[0-9]+$", non_na_values))) {
        message(paste("Converting labelled column to numeric:", col_name))
        data[[col_name]] <- as.numeric(data[[col_name]])
      } else {
        message(paste("Converting labelled column to factor:", col_name))
        data[[col_name]] <- haven::as_factor(data[[col_name]], levels = "label")
      }
    }
  } else {
    message("No labelled columns found to process.")
  }
  
  # Step 2: Process character columns
  char_columns <- names(data)[purrr::map_lgl(data, is.character)]
  
  if (length(char_columns) > 0) {
    message("Processing character columns:")
    # print(char_columns)
    for (col_name in char_columns) {
      # Check if all non-NA values in the column are numeric-like
      non_na_values <- data[[col_name]][!is.na(data[[col_name]])]
      if (all(grepl("^[0-9]+$", non_na_values))) {
        message(paste("Converting character column to numeric:", col_name))
        data[[col_name]] <- as.numeric(data[[col_name]])
      } else {
        message(paste("Converting character column to factor:", col_name))
        data[[col_name]] <- as.factor(data[[col_name]])
      }
    }
  } else {
    message("No character columns found to process.")
  }
...
# Recursively flatten sublists, always prefixing with the accumulated parent
flatten_dfs <- function(x, parent) {
  out <- list()
  nms <- names(x) %||% as.character(seq_along(x))
  for (i in seq_along(x)) {
    el <- x[[i]]
    nm <- nms[[i]]
    key <- paste(parent, nm, sep = ".")
    if (is.data.frame(el)) {
      out[[key]] <- el
    } else if (is.list(el)) {
      out <- c(out, flatten_dfs(el, parent = key))
    }
  }
  out
}
...
##############################################
# Updated summarise_data function (fixed fun_numeric names)
##############################################
...
summarise_data <- function(data_list, 
                           ...
                           perl              = TRUE, 
                           render_to_viewer  = FALSE, 
                           include_all_data  = FALSE, 
                           var_dict          = NULL,
                           use_skim          = TRUE,          # TRUE = datasummary_skim; FALSE = datasummary
                           include_histogram = FALSE) {       # default: no histogram
  
  # exactly one of filter_pattern / selected_columns
  if (!xor(is.null(filter_pattern), is.null(selected_columns))) {
    stop("You must provide either a filter_pattern or selected_columns, but not both.")
  }
  if (save_to_file && missing(output_directory)) {
    stop("You must provide an output_directory when save_to_file is TRUE.")
  }
  
  # helpers to detect "valid" columns that won't be dropped by skim
  has_non_missing <- function(x) any(!is.na(x))
  is_valid_numeric <- function(x) is.numeric(x) && has_non_missing(x)
  is_valid_categorical <- function(x) {
    if (is.logical(x)) {
      return(has_non_missing(x))
    } else if (is.factor(x)) {
      return(nlevels(x) <= 50 && has_non_missing(x))
    } else if (is.character(x)) {
      ux <- unique(x[!is.na(x)])
      return(length(ux) <= 50 && length(ux) > 0)
    }
    FALSE
  }
  ...
  for (data_name in names(data_list)) {
    ...
    if (!is.null(selected_columns)) {
      keep <- grepl(selected_columns, names(data), perl = perl)
    } else {
      keep <- !grepl(filter_pattern, names(data), perl = perl)
    }
    if (!any(keep)) {
      warning(paste("No matching columns found in", data_name, ". Skipping."))
      next
    }
    data <- dplyr::select(data, dplyr::all_of(names(data)[keep]))
    
    # --- Convert labelled & character columns ---
    data <- convert_labels_replace(data)
    
    # --- Optionally restrict to numeric only ---
    if (!include_all_data) {
      data <- dplyr::select(data, dplyr::where(is.numeric))
      if (nrow(data) == 0 || ncol(data) == 0) {
        warning(paste("No numeric variables found in", data_name, ". Skipping."))
        next
      }
    }
    ...
    if (!is.null(var_dict)) {
      for (col_name in names(data)) {
        if (col_name %in% names(var_dict)) {
          attr(data[[col_name]], "label") <- var_dict[[col_name]]
        }
      }
    }
    
    # --- Detect valid types now (post-filtering) ---
    has_num_valid <- any(vapply(data, is_valid_numeric, logical(1)))
    has_cat_valid <- any(vapply(data, is_valid_categorical, logical(1)))
    
    # --- Hard skip if neither present (codebook / unsupported types) ---
    if (!has_num_valid && !has_cat_valid) {
      warning(paste(
        "Skipping", data_name, 
        ": no usable numeric or categorical variables after filtering/conversion (likely a codebook or high-cardinality/empty columns)."
      ))
      next
    }
    ...
    # --- Summarise ---
    if (isTRUE(use_skim)) {
      # Build datasummary_skim() arguments dynamically
      args <- list(
        data   = data,
        type   = if (has_num_valid && has_cat_valid) "all" else if (has_num_valid) "numeric" else "categorical",
      ...
      if (has_num_valid) {
        ...
        if (isTRUE(include_histogram)) fn_num[["Histogram"]] <- Histogram
        args$fun_numeric <- fn_num
      }
      
      if (has_cat_valid) {
        args$fun_categorical <- list(
          "Unique"      = NUnique,
          "Missing Pct" = function(x) mean(is.na(x)) * 100
        )
      }
      
      summary_output <- do.call(modelsummary::datasummary_skim, args)
      
    } else {
      # datasummary(): work on numeric-only & define Missing
      data_num <- dplyr::select(data, dplyr::where(is.numeric))
      if (ncol(data_num) == 0 || !any(vapply(data_num, has_non_missing, logical(1)))) {
        warning(paste("Skipping", data_name, ": datasummary requires at least one numeric variable with non-missing values."))
        next
      }
      Missing <- function(x) sum(is.na(x))
      summary_output <- modelsummary::datasummary(
        All(data_num) ~ Mean + SD + Min + Max + N + Missing,
        data   = data_num,
        output = output_format,
        title  = data_name
      )
    ...
    if (render_to_viewer && output_format == "html" && interactive()) {
    ...
    if (save_to_file) {
      ...
      if (output_format == "html") {
        writeLines(as.character(summary_output), summary_file)
      } else if (output_format %in% c("markdown", "latex", "text")) {
        writeLines(capture.output(print(summary_output)), summary_file)
      } else {
...
#############################################
# Generic file-saving helpers (ported from
# the former Week4/Custom_scripts/custom_functions.R).
#############################################
...
save_data_simple <- function(data, file_name, file_type) {
  switch(tolower(file_type),
         ...
         "sav" = {
           haven::write_sav(data, paste0(file_name, ".sav"))
         },
         "feather" = {
           arrow::write_feather(data, paste0(file_name, ".feather"))
         },
         ...
         {
           logger::log_warn(paste("Unsupported file type:", file_type, "for file:", file_name))
         }
  )
}
